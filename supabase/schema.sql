...
insert into config (key, value, notes) values
  ...
  ('DESIGN_LIBRARY', '', 'What this raffle has made once and wants again, as JSON: saved shapes, named colours and named text styles. A saved shape holds its parts as shares of its OWN bounds, so it can be placed at any size. Blank means the four built-in shapes, which live in code so that improving one reaches every raffle.'),
