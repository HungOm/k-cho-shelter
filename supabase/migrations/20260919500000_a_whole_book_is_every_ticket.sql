-- A whole book is one where every ticket is still there.
--
-- Book-004 sat "In the office" with 10 of 10 sold and was still counted among
-- the books free to give out. The rule added that morning asked whether any
-- ticket had been SOLD, which caught that one — but "none sold" is not the same
-- as "all there". A book with tickets RESERVED or VOIDED in it has nothing to
-- carry to the wrong seller and is still not ten tickets somebody can sell.
--
-- So the test is that every ticket in the book is Available. That fails
-- reserved, voided, and any status invented later, without this line having to
-- learn their names.
--
-- The same rule is in the client, so the screen does not offer what this will
-- refuse — and it fails CLOSED there on a book whose counts have not loaded,
-- because handing somebody a book on a count you have not got is the wrong way
-- to be wrong.
--
-- AND THE REFUSAL FOR AN OFFERED BOOK NOW NAMES THE RIGHT WAY ROUND IT. An
-- offer has not been accepted, so nothing has to come back: the organiser takes
-- the offer back and the book is on the shelf again. Only a book that has been
-- ACCEPTED needs a return, or the seller's report at the deadline. Telling
-- somebody to have an offered book "brought back" sends them to a screen that
-- cannot help them.
--
-- DATA LOSS RISK: NO. Three functions are replaced. No row is read or written.
