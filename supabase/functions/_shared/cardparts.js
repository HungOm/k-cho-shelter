/*
 * MOVED HERE FROM src/lib/cardelements.js ON 2026-09-23 (STUDIO-ESSENTIALS
 * Phase 9), unchanged but for this note and one import path.
 *
 * The server now needs it: a card can carry drawn shapes, and the one rule
 * that matters for them — nothing drawn over the QR — needs to know where each
 * treatment puts its QR, which only this table knows. The client keeps its
 * import path through a re-export, the arrangement designelements.js and
 * designlibrary.js already use, and `.js` for the same reason as theirs.
 */
/**
 ...
 */
import { CARD_TREATMENT_IDS } from './cardtreatments.js'
