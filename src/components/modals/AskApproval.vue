...
<template>
  <Sheet title="This needs the System Admin" @close="emit('close')">
    <template v-if="sent">
      <div class="center" style="padding:14px 0">
        <div style="font-size:2.6rem">📨</div>
        <h2>Asked</h2>
        <p class="muted">
          The System Admin has been asked. Nothing has changed yet — it happens only
          when they say yes.
        </p>
    ...
    <template v-else>
      ...
      <p>
        Changes this big need two people. You can ask the System Admin to approve it —
    ...
    <template #actions>
      ...
      <template v-else>
        ...
        <button class="btn primary" :disabled="busy" @click="ask">
          {{ busy ? 'Asking…' : 'Ask the System Admin' }}
        </button>
      </template>
    </template>
  </Sheet>
